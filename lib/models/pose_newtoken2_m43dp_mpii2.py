...
class CrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        q = self.wq(x[:, 0:16, ...]).reshape(B, 16, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # B1C -> B1H(C/H) -> BH1(C/H)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, 16, C)   # (BH1N @ BHN(C/H)) -> BH1(C/H) -> B1H(C/H) -> B1C
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        x = x[:, 0:16, ...] + self.drop_path(self.attn(self.norm1(x)))
        if self.has_mlp:
                ...
                if i < len(self.num_token) -1:
                    self.layers.append(nn.ModuleList([
                        PreNorm(dim, AlternateAttention(
                            dim, [self.num_token[i] + 16, self.num_token[(i + 1)] + 16], num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                            proj_drop=drop)),
                    ...
                    self.layers.append(nn.ModuleList([
                        PreNorm(dim, AlternateAttention(
                            dim, [self.num_token[0] + 16, self.num_token[i] + 16], num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                            proj_drop=drop)),
                        Residual(PreNorm(dim, FeedForward(dim, mlp_dim, dropout=dropout)))
                    ]))
        ...
        for d in range(len(self.layers)//len(self.num_token)):
            for i in range(len(self.num_token)):
                ...
                if i < len(self.num_token) -2:
                    ...
                    k2, x[i + 1] = x[i + 1].split([16, self.num_token[i + 1]], dim=1)
                    k1, x[i] = x[i].split([16,self.num_token[i]], dim=1)
                    ...
                    k2, x[i] = x[i].split([16, self.num_token[i]], dim=1)
...
class SenTrans(nn.Module):
    def __init__(self, *, branch, feature_size, patch_size, num_keypoints, dim, depth, scale_depth, heads, mlp_dim, apply_init=False,
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(dim*2),
            nn.Linear(dim*2, hidden_heatmap_dim),
            nn.LayerNorm(hidden_heatmap_dim),
            nn.Linear(hidden_heatmap_dim, heatmap_dim)
        ) if (dim*2 <= hidden_heatmap_dim * 0.5) else nn.Sequential(
            nn.LayerNorm(dim*2),
            nn.Linear(dim*2, heatmap_dim)
        )
    ...
    def forward(self, feature, mask=None):
        ...
        for i in range(0, len(feature)):
            ...
            x_set.append(x)
        # print('x_set------->', [x.shape for x in x_set])
        # print('pos_attn------->', [x.shape for x in pos_attn if x is not None])
        ...
        x = self.mlp_head(torch.cat((k1, k2), dim=-1))
...
class CAPose(nn.Module):
    ...
    def forward(self, x):
        x = self.pre_feature(x)                             # # torch.Size([1, 48, 64, 48]) torch.Size([1, 96, 32, 24])
        # print(x.shape)          # torch.Size([1, 48, 64, 48])
        x = self.transformer(x)
        # print(x.shape)          # torch.Size([1, 17, 64, 48])
        return x
